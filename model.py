...
class UnknownSample(Sample):
    ...
    def from_dict(cls,row: dict[str,str]) -> "UnknownSample":
        if set(row.keys()) != {
            ...
            raise InvaildSampleError(f"invalid fields in {row!r}")
        try:
            return cls(
                sepal_length=float(row["sepal_length"])
                sepal_width=float(row["sepal_width"])
                petal_length=float(row["petal_pass
...
class TestingKnownSample(KnownSample):
    @classmethod
    def from.dict_(cls,row:dict[str,str]) -> "TrainingKnownSample" :
        return 
    def __init__(
        self,
        species: str,
    ...
    def from_dict(cls,row: dict[str,str]) -> "TestingKnownSample":
        return cast(TestingKnownSample, super().from_dict)
